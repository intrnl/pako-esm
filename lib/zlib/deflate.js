...
import { U16, U8 } from '../utils/common.js';
...
let MIN_MATCH = 3;
let MAX_MATCH = 258;
let MIN_LOOKAHEAD = (MAX_MATCH + MIN_MATCH + 1);

let PRESET_DICT = 0x20;

let INIT_STATE = 42;
let EXTRA_STATE = 69;
let NAME_STATE = 73;
let COMMENT_STATE = 91;
let HCRC_STATE = 103;
let BUSY_STATE = 113;
let FINISH_STATE = 666;

let BS_NEED_MORE      = 1; /* block not completed, need more input or more output */
let BS_BLOCK_DONE     = 2; /* block flush performed */
let BS_FINISH_STARTED = 3; /* finish started, need only more output at next deflate */
let BS_FINISH_DONE    = 4; /* finish done, accept no more input or output */

let OS_CODE = 0x03; // Unix :) . Don't detect, use this default.
...
class DeflateState {
  ...
  gzhead = null;         /* gzip header information to write */
  ...
  head = null;   /* Heads of the hash chains or NIL. */
  ...
  dyn_ltree  = new U16(HEAP_SIZE * 2);
  dyn_dtree  = new U16((2 * D_CODES + 1) * 2);
  bl_tree    = new U16((2 * BL_CODES + 1) * 2);
  ...
  bl_count = new U16(MAX_BITS + 1);
  /* number of codes at each bit length for an optimal tree */

  //int heap[2*L_CODES+1];      /* heap used to build the Huffman trees */
  heap = new U16(2 * L_CODES + 1);  /* heap used to build the Huffman trees */
  ...
  depth = new U16(2 * L_CODES + 1); //uch depth[2*L_CODES+1];
...
export function deflateInit2 (strm, level, method, windowBits, memLevel, strategy) {
  ...
  s.window = new U8(s.w_size * 2);
  s.head = new U16(s.hash_size);
  s.prev = new U16(s.w_size);
  ...
  s.pending_buf = new U8(s.pending_buf_size);
